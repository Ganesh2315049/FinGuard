...
public class AuthController {
    ...
    @PostMapping("/login") public Object login(@Valid @RequestBody LoginRequest r){User u=users.byEmail(r.email()); if(!u.isEnabled()||!encoder.matches(r.password(),u.getPassword())) throw new IllegalArgumentException("Invalid email or password"); return java.util.Map.of("token",jwt.create(u.getId(),u.getEmail(),u.getRole().name()),"user",java.util.Map.of("name",u.getName(),"email",u.getEmail(),"role",u.getRole()));}
}
